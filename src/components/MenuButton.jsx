...
export function MenuButton({ id, href, title, subtitle, onClick }) {
	...
	return (
		<motion.a
			...
			{title}
			<span className="absolute bottom-8  text-[0.5rem] md:text-xs text-[#321d20] bg-white/50 px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition text-center pointer-events-none">
				{subtitle}
			</span>
		</motion.a>
	);
